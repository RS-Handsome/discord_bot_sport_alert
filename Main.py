                    ...
                    if q_no < match_league["round"] or (match_league["short_name"] == "CBA" and q_no == match_league["round"]):                    
                        if updated_remain > LIMIT_TIME_QUARTER_TOTAL:
                            ...
                            if schedules[QUARTER_TOTAL][id][q_no]["timeout_count"] >= 5:
                                if updated_remain < 30 or (match_league["short_name"] == "CBA" and q_no == 4) or (match_league["short_name"] != "CBA" and q_no == 3):
        ...
        while True:
            ...
            if len(buttons) == 1:
                ...
                for index, match_element in enumerate(match_elements):
                    ...
                    log(f'{str(index + 1).zfill(2)}. [{unidecode(home_team).center(30)}] vs [{unidecode(away_team).center(30)}] : {output}\n')
                break
